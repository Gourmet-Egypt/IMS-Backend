...
<head>
    ...
    {{--
        Overridden from vendor to fix Telescope when Laravel runs in a
        sub-directory (e.g. http://host/IMS). Telescope's default js() sets
        window.Telescope.path straight from config('telescope.path') = "telescope",
        so the Vue router base and every telescope-api call resolve to
        "/telescope/..." at the web root and 404 — leaving a blank page.

        We instead prefix the base path Laravel itself detected via getBaseUrl()
        (e.g. "/IMS"), producing "IMS/telescope". Backend route registration is
        unchanged (still "telescope"), so this only corrects the front-end URLs
        and automatically collapses to "telescope" if the app is ever served
        from the web root.
    --}}
    @php
        $telescopeVariables = [
            'path' => trim(request()->getBaseUrl() . '/' . config('telescope.path'), '/'),
            'timezone' => config('app.timezone'),
            'recording' => ! cache('telescope:pause-recording'),
        ];
    @endphp
    <script type="module">
        window.Telescope = {!! \Illuminate\Support\Js::from($telescopeVariables) !!};
        {!! file_get_contents(base_path('vendor/laravel/telescope/dist/app.js')) !!}
    </script>
</head>
<body>
